...
"</div>"+'<div class="rev_sched_opt">'+'<input type="radio" name="UserReview" value="Yes" class="schedule_opt user_review_radio"/>'+"</div>"+'<div class="review disabled">'+'<label for="user_review_radio"><%= this.userReviewLabel %></label>'+"</div>"+"</div>"+"<% } %>"+'<div class="buttons">'+"<% if ( this.buttonsMessage ) { %>"+'<div class="buttonMessage message"><%= this.buttonsMessage %></div>'+"<% } %>"+'<% if ( this.submitButtonsTag === "input" ) { %>'+'<input type="submit" value="<%= this.submitButtonLabel %>" class="dialog_submit"/>'+
...
settings.cleanValues(document.forms[settings.form][settings.globalFields[x].name].value);this.data("aeonRequestsDialog",{settings:settings})},_compressRequests:function(){var settings=this.data("aeonRequestsDialog").settings;var codes={};var idSelector="#"+settings.dialogId;var fieldName=settings.compressRequestsField;var requestsSelector=idSelector+" "+settings.requestsSelector+":checked";var requests=$(requestsSelector);var newReqNum=requests.length;requests.each(function(){var requestNumber=$(this).val();
var collapseFieldValue=$(idSelector+' input[name="'+fieldName+"_"+requestNumber+'"]').val();if(!codes[collapseFieldValue])codes[collapseFieldValue]=new Array;codes[collapseFieldValue].push(requestNumber)});for(var code in codes){var requestNumbers=codes[code];if(requestNumbers.length<2)continue;var newRequest={};for(var x=0;x<settings.itemFields.length;x++){var field=settings.itemFields[x].name;for(var y=0;y<requestNumbers.length;y++){var key=requestNumbers[y];var oldField=$(idSelector+' input[name="'+
field+"_"+key+'"]');if(!oldField)continue;var oldVal=oldField.val();if(!newRequest[field])newRequest[field]=oldVal;else if(newRequest[field]!=oldVal)newRequest[field]+="; "+oldVal;oldField.remove();$('input[name="Request"][value="'+key+'"]').remove()}}$("<input/>").prop("type","hidden").prop("name","Request").val(newReqNum).appendTo(idSelector+" .aeon_request_form");for(var field in newRequest)$("<input/>").prop("type","hidden").prop("name",field+"_"+newReqNum).val(newRequest[field]).appendTo(idSelector+
" .aeon_request_form");newReqNum++}},_stripUnchecked:function(){var settings=this.data("aeonRequestsDialog").settings;var idSelector="#"+settings.dialogId;var uncheckedSelector=idSelector+" "+settings.requestsSelector+":not(:checked)";$(uncheckedSelector).each(function(){$(idSelector+' input[name$="_'+$(this).val()+'"]').remove()})},options:function(){var settings=this.data("aeonRequestsDialog").settings;if(arguments.length===0)return settings;else if(arguments.length===1)if(typeof arguments[0]===
"object")settings=$.extend(settings,arguments[0]);else if(typeof arguments[0]==="string"){if(undefined===settings[arguments[0]]){console.log("Unknown key sent to arguments.");return null}return settings[arguments[0]]}else{console.log("Argument to options must be of type Object or String");return null}else if(arguments.length===2)settings[arguments[0]]=arguments[1];else{console.log("Illegal number of arguments sent to options.");return null}this.data("aeonRequestsDialog",{settings:settings});return settings},
destroy:function(){var data=this.data("aeonRequestsDialog");$(window).off(".aeonRequestsDialogMain");$this.removeData("aeonRequestsDialog");return this}};$.fn.aeonRequestsDialog=function(method){if(methods[method])return methods[method].apply(this,Array.prototype.slice.call(arguments,1));else if(typeof method==="object"||!method)return methods.init.apply(this,arguments);else{$.error("Method "+method+" does not exist on jQuery.aeonRequestDialog");return null}}})(jQuery);